...
//The average of the **changes** in Profit/Losses over the entire period.
  ...
  var diffTotal = 0;
    ...
    for (var i=1; i < finances.length; i++){
        ...
        var diffTotal = diffTotal + diff;
        finances[i][2] = diff; // new culumns add to fianaces which equil the difference between each month
    }

var averageChange = (diffTotal/(finances.length - 1));

//The greatest increase in Profit/Losses (date and difference in the amounts) over the entire period.


var maxIncrease = finances[1][2];
var maxIncreaseMonth = finances[1][0];
for (var i=1; i < finances.length; i++){
    if (finances[i][2] > maxIncrease){
        var maxIncrease = finances[i][2]; // asign the highest number to maxIncrease
        var maxIncreaseMonth = finances[i][0]; // asign the period with the hight increase to maxIncreaseMonth
    }
}
...
var maxDecrease = finances[1][2];
var maxDecreaseMonth = finances[1][0];

for (var i=1; i < finances.length; i++){
    if (finances[i][2] < maxDecrease){
        var maxDecrease = finances[i][2]; // asign the lowest number to maxIncrease
        var maxDecreaseMonth = finances[i][0]; //// asign the period with the lowest amount to maxIncreaseMonth
    }
}
